...
The `testing-utilities` capability covers the package's test helpers under `Longman\LaravelLodash\Testing`. It defines the extended `Response` (a `TestResponse` subclass) asserting the `{status, message, data, meta}` JSON envelope: collection assertions that validate every row behind a guard chain, opt-in exact key-set matching scoped to the `data` and `meta.pagination` subtrees, and combined resource assertions that additionally assert the wire `type` and concrete model identity. It also defines the `DataStructuresProvider` base class that resolves named data structures with a validated relation include grammar: dots for linear chains and nested arrays for branching over one internal tree, `name[]` collection markers, and order-independent merging of overlapping declarations.
